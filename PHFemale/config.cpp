class CfgPatches
{
	class PHRP_Female
	{
		author="PHRP Mod Team";
		units[]={};
		weapons[]={};
		requiredVersion=0.1;
		requiredAddons[]=
		{
			"A3_Characters_F"
		};
	};
};
class cfgWeapons
{
	class HeadgearItem;
	class ItemCore;
	class PHRP_hair_ash_helmet: ItemCore
	{
		scope=2;
		weaponPoolAvailable=1;
		displayName="(Female) Hair 1";
		model="PHFemale\hair\Hair_ash.p3d";
		class ItemInfo: HeadgearItem
		{
			mass=30;
			uniformmodel="PHFemale\hair\Hair_ash.p3d";
			modelSides[]={6};
			material=-1;
			explosionShielding=0;
			minimalHit=0.0099999998;
			passThrough=0.0099999998;
			class HitpointsProtectionInfo
			{
				class Head
				{
					hitpointName="HitHead";
					armor=0;
					passThrough=0.1;
				};
			};
		};
	};
	class PHRP_hair_cls_helmet: ItemCore
	{
		scope=2;
		weaponPoolAvailable=1;
		displayName="(Female) Hair 2";
		model="PHFemale\hair\Hair_cls.p3d";
		class ItemInfo: HeadgearItem
		{
			mass=30;
			uniformmodel="PHFemale\hair\Hair_cls.p3d";
			modelSides[]={6};
			material=-1;
			explosionShielding=0;
			minimalHit=0.0099999998;
			passThrough=0.0099999998;
			class HitpointsProtectionInfo
			{
				class Head
				{
					hitpointName="HitHead";
					armor=0;
					passThrough=0.1;
				};
			};
		};
	};
	class PHRP_hair_cte_helmet: ItemCore
	{
		scope=2;
		weaponPoolAvailable=1;
		displayName="(Female) Hair 3";
		model="PHFemale\hair\Hair_cte.p3d";
		class ItemInfo: HeadgearItem
		{
			mass=30;
			uniformmodel="PHFemale\hair\Hair_cte.p3d";
			modelSides[]={6};
			material=-1;
			explosionShielding=0;
			minimalHit=0.0099999998;
			passThrough=0.0099999998;
			class HitpointsProtectionInfo
			{
				class Head
				{
					hitpointName="HitHead";
					armor=0;
					passThrough=0.1;
				};
			};
		};
	};
	class PHRP_hair_hbn_helmet: ItemCore
	{
		scope=2;
		weaponPoolAvailable=1;
		displayName="(Female) Hair 4";
		model="PHFemale\hair\Hair_hbn.p3d";
		class ItemInfo: HeadgearItem
		{
			mass=30;
			uniformmodel="PHFemale\hair\Hair_hbn.p3d";
			modelSides[]={6};
			material=-1;
			explosionShielding=0;
			minimalHit=0.0099999998;
			passThrough=0.0099999998;
			class HitpointsProtectionInfo
			{
				class Head
				{
					hitpointName="HitHead";
					armor=0;
					passThrough=0.1;
				};
			};
		};
	};
	class PHRP_hair_jes_helmet: ItemCore
	{
		scope=2;
		weaponPoolAvailable=1;
		displayName="(Female) Hair 5";
		model="PHFemale\hair\Hair_jes.p3d";
		class ItemInfo: HeadgearItem
		{
			mass=30;
			uniformmodel="PHFemale\hair\Hair_jes.p3d";
			modelSides[]={6};
			material=-1;
			explosionShielding=0;
			minimalHit=0.0099999998;
			passThrough=0.0099999998;
			class HitpointsProtectionInfo
			{
				class Head
				{
					hitpointName="HitHead";
					armor=0;
					passThrough=0.1;
				};
			};
		};
	};
	class PHRP_hair_mir_helmet: ItemCore
	{
		scope=2;
		weaponPoolAvailable=1;
		displayName="(Female) Hair 6";
		model="PHFemale\hair\Hair_mir.p3d";
		class ItemInfo: HeadgearItem
		{
			mass=30;
			uniformmodel="PHFemale\hair\Hair_mir.p3d";
			modelSides[]={6};
			material=-1;
			explosionShielding=0;
			minimalHit=0.0099999998;
			passThrough=0.0099999998;
			class HitpointsProtectionInfo
			{
				class Head
				{
					hitpointName="HitHead";
					armor=0;
					passThrough=0.1;
				};
			};
		};
	};
	class PHRP_hair_shep_helmet: ItemCore
	{
		scope=2;
		weaponPoolAvailable=1;
		displayName="(Female) Hair 7";
		model="PHFemale\hair\Hair_shep.p3d";
		hiddenSelections[] = {"camo"};
		hiddenSelectionsTextures[] = {"PHFemale\hair\shep_pa.paa"};
		class ItemInfo: HeadgearItem
		{
			mass=30;
			uniformmodel="PHFemale\hair\Hair_shep.p3d";
			hiddenSelections[] = {"camo"};
			hiddenSelectionsTextures[] = {"PHFemale\hair\shep_pa.paa"};
			modelSides[]={6};
			material=-1;
			explosionShielding=0;
			minimalHit=0.0099999998;
			passThrough=0.0099999998;
			class HitpointsProtectionInfo
			{
				class Head
				{
					hitpointName="HitHead";
					armor=0;
					passThrough=0.1;
				};
			};
		};
	};
	class PHRP_hair_ff_helmet: ItemCore
	{
		scope=2;
		weaponPoolAvailable=1;
		displayName="(Female) Hair 8";
		model="PHFemale\hair\Hair_ff.p3d";
		class ItemInfo: HeadgearItem
		{
			mass=30;
			uniformmodel="PHFemale\hair\Hair_ff.p3d";
			modelSides[]={6};
			material=-1;
			explosionShielding=0;
			minimalHit=0.0099999998;
			passThrough=0.0099999998;
			class HitpointsProtectionInfo
			{
				class Head
				{
					hitpointName="HitHead";
					armor=0;
					passThrough=0.1;
				};
			};
		};
	};
};
class CfgFaces
{
	class Default;
	class Man_A3: Default
	{
		class Default;
		class Head_Female_As1: Default
		{
			name = "FemaleA1";
			displayname = "Female Asian 1";
			identityTypes[] = {"HeadFemaleAsian"};
			head = "Head_FemaleAs_head";
			texture = "PHFemale\isabel_head_co.paa";
			disabled = 0;
			author = "Opposition Team";
			material = "PHFemale\isabel.rvmat";
			textureHL = "PHFemale\data\hl_asa_nkd_white_co.paa";
			materialHL = "PHFemale\data\hl_asa_nkd.rvmat";
			textureHL2 = "PHFemale\data\hl_asa_nkd_white_co.paa";
			materialHL2 = "PHFemale\data\hl_asa_nkd.rvmat";
		};
		class Head_Female_Eu1: Default
		{
			name = "FemaleE1";
			displayname = "Female Europe 1";
			identityTypes[] = {"HeadFemaleEurope"};
			head = "Head_FemaleEu_head";
			texture = "PHFemale\yennefer_head_co.paa";
			disabled = 0;
			author = "Opposition Team";
			material = "PHFemale\yennefer.rvmat";
			textureHL = "PHFemale\data\hl_asa_nkd_white_co.paa";
			materialHL = "PHFemale\data\hl_asa_nkd.rvmat";
			textureHL2 = "PHFemale\data\hl_asa_nkd_white_co.paa";
			materialHL2 = "PHFemale\data\hl_asa_nkd.rvmat";
		};
		class Head_Female_Eu2: Default
		{
			name = "FemaleE2";
			displayname = "Female Europe 2";
			identityTypes[] = {"HeadFemaleEurope"};
			head = "Head_FemaleEu2_head";
			texture = "PHFemale\ciri_head_co.paa";
			disabled = 0;
			author = "Opposition Team";
			material = "PHFemale\ciri.rvmat";
			textureHL = "PHFemale\data\hl_asa_nkd_white_co.paa";
			materialHL = "PHFemale\data\hl_asa_nkd.rvmat";
			textureHL2 = "PHFemale\data\hl_asa_nkd_white_co.paa";
			materialHL2 = "PHFemale\data\hl_asa_nkd.rvmat";
		};
		class PHRP_female_head_1_01: Default
		{
			author = "Opposition Team";
			displayname = "Female 1";
			head = "PHRP_female_head_1";
			identityTypes[] = {"HeadFemaleEurope"};
			texture = "PHFemale\New\fhead1_co.paa";
			material = "PHFemale\New\fhead1_white.rvmat";
			textureHL = "PHFemale\data\hl_asa_nkd_white_co.paa";
			materialHL = "PHFemale\data\hl_asa_nkd.rvmat";
			textureHL2 = "PHFemale\data\hl_asa_nkd_white_co.paa";
			materialHL2 = "PHFemale\data\hl_asa_nkd.rvmat";
		};
		class PHRP_female_head_1_02: Default
		{
			author = "Opposition Team";
			displayname = "Female 2";
			head = "PHRP_female_head_1";
			identityTypes[] = {"HeadFemaleEurope"};
			texture = "PHFemale\New\fhead2_co.paa";
			material = "PHFemale\New\fhead2_white.rvmat";
			textureHL = "PHFemale\data\hl_asa_nkd_white_co.paa";
			materialHL = "PHFemale\data\hl_asa_nkd.rvmat";
			textureHL2 = "PHFemale\data\hl_asa_nkd_white_co.paa";
			materialHL2 = "PHFemale\data\hl_asa_nkd.rvmat";
		};
		class PHRP_female_head_1_03: Default
		{
			author = "Opposition Team";
			displayname = "Female 3";
			head = "PHRP_female_head_1";
			identityTypes[] = {"HeadFemaleEurope"};
			texture = "PHFemale\New\fhead3_co.paa";
			material = "PHFemale\New\fhead3_white.rvmat";
			textureHL = "PHFemale\data\hl_asa_nkd_white_co.paa";
			materialHL = "PHFemale\data\hl_asa_nkd.rvmat";
			textureHL2 = "PHFemale\data\hl_asa_nkd_white_co.paa";
			materialHL2 = "PHFemale\data\hl_asa_nkd.rvmat";
		};
		class PHRP_female_head_1_04: Default
		{
			author = "Opposition Team";
			displayname = "Female 4";
			head = "PHRP_female_head_1";
			identityTypes[] = {"HeadFemaleEurope"};
			texture = "PHFemale\New\fhead4_co.paa";
			material = "PHFemale\New\fhead4_white.rvmat";
			textureHL = "PHFemale\data\hl_asa_nkd_white_co.paa";
			materialHL = "PHFemale\data\hl_asa_nkd.rvmat";
			textureHL2 = "PHFemale\data\hl_asa_nkd_white_co.paa";
			materialHL2 = "PHFemale\data\hl_asa_nkd.rvmat";
		};
	};
};
class CfgHeads
{
	class Default_A3;
	class Head_FemaleAs_head: Default_A3
	{
		model = "PHFemale\isabel_1.p3d";
		class Wounds
		{
			tex[] = {};
			mat[] = {"PHFemale\isabel.rvmat"};
		};
	};
	class Head_FemaleEu_head: Default_A3
	{
		model = "PHFemale\yennefer_1.p3d";
		class Wounds
		{
			tex[] = {};
			mat[] = {"PHFemale\yennefer.rvmat"};
		};
	};
	class Head_FemaleEu2_head: Default_A3
	{
		model = "PHFemale\ciri_1.p3d";
		class Wounds
		{
			tex[] = {};
			mat[] = {"PHFemale\ciri.rvmat"};
		};
	};
	class PHRP_female_head_1: Default_A3
	{
		model = "PHFemale\female_head_1.p3d";
		class Wounds
		{
			tex[] = {};
			mat[] = {};
		};
	};
};
class CfgIdentities
{
	class HeadFemaleAsian
	{
		name = "Female head (asian)";
		pitch = 1;
		nameSound = "Kerry";
	};
	class HeadFemaleEurope
	{
		name = "Female head (europe)";
		pitch = 1;
		nameSound = "Kerry";
	};
};