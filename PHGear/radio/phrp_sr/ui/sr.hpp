...
class phrp_sr_radio_dialog {
    idd = IDD_PHRP_SR_RADIO_DIALOG;
    movingEnable = 1;
    controlsBackground[] = { };
    objects[] = { };
    onUnload = "['OnRadioOpen', [player, TF_sw_dialog_radio, false, 'phrp_sr_radio_dialog', false]] call TFAR_fnc_fireEventHandlers;";
    onLoad = QUOTE(if (sunOrMoon < 0.2) then {((_this select 0) displayCtrl TF_IDD_BACKGROUND) ctrlSetText 'PATHTOF(phrp_sr\ui\HUD_SR.paa)';};);
    controls[]= {
        background,
        enter,
        edit,
    ...
    class background: RscBackPicture {
        idc = IDC_PHRP_SR_BACKGROUND;
        text = QPATHTOF(phrp_sr\ui\HUD_SR.paa);
    ...
    class edit: RscEditLCD {
        ...
        onKeyUp = QUOTE( \
            if (_this select 1 in [ARR_2(28,156)]) then { \
                [((ctrlParent (_this select 0))) displayCtrl IDC_PHRP_SR_EDIT] call TFAR_handhelds_fnc_onButtonClick_Enter; \
            }; \
        );
    };
    class channel_edit: RscEditLCD {
        moving = 1;
        idc = IDC_PHRP_SR_CHANNEL_EDIT;
    ...
    class enter: HiddenButton {
        ...
        onButtonClick = QUOTE([((ctrlParent (_this select 0))) displayCtrl IDC_PHRP_SR_EDIT] call TFAR_handhelds_fnc_onButtonClick_Enter;);
        action = "";
    };
    class clear: HiddenButton {
        idc = IDC_PHRP_SR_CLEAR;
        ...
        action = QUOTE( \
            ctrlSetText [ARR_2(IDC_PHRP_SR_EDIT, '')]; \
            ctrlSetFocus ((findDisplay IDD_PHRP_SR_RADIO_DIALOG) displayCtrl IDC_PHRP_SR_EDIT); \
        );
    };
    class additional: HiddenButton {
        idc = IDC_PHRP_SR_RADIO_DIALOG_ADDITIONAL;
